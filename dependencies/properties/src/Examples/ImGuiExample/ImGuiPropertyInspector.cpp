...
namespace property::editor::details
{
    ...
    void draw<oobb, style::defaulted>(undo::cmd<oobb>& Cmd, const oobb& Value, const property::editor::style_info<oobb>&, property::flags::type Flags) noexcept
    {
        ...
        const auto   Width   = ( ImGui::GetContentRegionAvailWidth() - style->ItemInnerSpacing.x ) / 2;
...
{
    m_lEntities.push_back(std::unique_ptr<entity>{ new entity });
}
...
void property::inspector::AppendEntityComponent(const property::table& Table, void* pBase) noexcept
{
    ...
    m_lEntities.back()->m_lComponents.push_back(std::unique_ptr<component>{ &Component });
...
void property::inspector::RefreshAllProperties( void ) noexcept
{
    for ( auto& E : m_lEntities )
    {
        for ( auto& C : E->m_lComponents )
        {
            ...
            {
                entry* pEntry = new entry { std::string{ PropertyName }, Data, &Table.m_pEntry[ Index ], Flags };
                C->m_List.push_back( std::unique_ptr<entry>{ pEntry } );
            } );
        }
    }
}
...
void property::inspector::Render( component& C, int& GlobalIndex ) noexcept
{
    ...
    auto                        PushTree = [&]( std::uint32_t UID, const char* pName, std::size_t iStart, std::size_t iEnd, bool isReadOnly, bool bArray = false, bool bAtomic = false )
    {
        ...
        if( Open )
        {
            if ( iDepth >0 && Tree[iDepth-1].m_OpenAll ) ImGui::SetNextTreeNodeOpen( Tree[iDepth-1].m_OpenAll > 0 );
    ...
    //
    {
        ...
        ImVec2 pos = ImGui::GetCursorScreenPos();
        ImGui::GetWindowDrawList()->AddRectFilled( pos, ImVec2( pos.x + ImGui::GetContentRegionAvailWidth(), pos.y + ImGui::GetFrameHeight() ), ImGui::GetColorU32( ImGuiCol_Header ) );
        ImGui::PopStyleVar();
    }
    ...
    for ( std::size_t iE = 0; iE<C.m_List.size(); ++iE )
    {
        ...
        if ( E.m_Flags.m_isShowReadOnly || Tree[iDepth].m_isReadOnly )
        {
            ...
            ImGui::GetWindowDrawList()->AddRectFilled(lpos, ImVec2(lpos.x + ImGui::GetContentRegionAvailWidth(), lpos.y + ImGui::GetFrameHeight()), CC);
        }
        ...
        if( E.m_Flags.m_isScope || bRenderBlankRight )
        {
            ...
            {
                ImColor     CC = ImVec4(0.7f, 0.7f, 1.0f, 0.35f);
                ImGui::GetWindowDrawList()->AddRectFilled(rpos, ImVec2(rpos.x + ImGui::GetContentRegionAvailWidth(), rpos.y + ImGui::GetFrameHeight()), CC);
            }
        }
        else
        {
            ...
            if ( E.m_Flags.m_isShowReadOnly || Tree[iDepth].m_isReadOnly )
            {
                ...
                ImGui::GetWindowDrawList()->AddRectFilled( pos, ImVec2( pos.x + ImGui::GetContentRegionAvailWidth(), pos.y + ImGui::GetFrameHeight() ), CC );
            }
            else
            {
...
void property::inspector::DrawBackground( int Depth, int GlobalIndex ) const noexcept
{
    ...
    ImGui::GetWindowDrawList()->AddRectFilled(
        pos
        , ImVec2( pos.x + ImGui::GetContentRegionAvailWidth()
